class StateEditorGUI {
    ...
    async createProvinceBorders() {
        console.log('Creating province borders...');
                    ...
                    if (provinceId) {
                        ...
                        for (const {dx, dy} of neighbors) {
                            ...
                            if (nProvinceId && nProvinceId !== provinceId) {
                                // Light gray border
                                borderData.data[idx] = 180;
                                borderData.data[idx + 1] = 180;
                                borderData.data[idx + 2] = 180;
                    ...
                    if (provinceId) {
                        ...
                        for (const {dx, dy} of neighbors) {
                            ...
                            if (nProvinceId) {
                                ...
                                if (stateId !== nStateId) {
                                    // Black border for state boundaries
    ...
    async handleBorderEdit(provinceId, stateId) {
        ...
        } else if (this.selectedState && this.selectedState.id !== stateId) {
    ...
    async handlePaintOwner(provinceId, stateId) {
        ...
        if (!stateId) {
            alert('Province not in a state. Use Edit Borders mode first.');
            return;
        }
    ...
    render() {
        ...
        if (this.showProvinceBorders && this.provinceBordersImage) {
            this.ctx.drawImage(this.provinceBordersImage, 0, 0);
        }
        
        if (this.showStateBorders && this.stateBordersImage) {
    ...
    async quickRefresh() {
        // Refresh data without recreating images
        const initResponse = await fetch('/api/state_editor/initialize', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' }
        });
        ...
        // Recreate only the stylized map and borders
        await this.createStylizedMap();
        await this.createStateBorders();
        
        this.render();
        
        // Update properties panel if a state is selected
    ...
    renderPropertiesPanel() {
        ...
        panel.html(`
            ...
            ${['infrastructure', 'industrial_complex', 'air_base', 'naval_base', 'synthetic_refinery', 'fuel_silo'].map(bld => `
                <div class="mb-2">
    ...
    showCreateStateModal() {
        ...
        $('#confirm-create-state').on('click', async () => {
            ...
            if (result.success) {
                ...
                $('#create-state-modal').remove();
                await this.quickRefresh();
                alert(`State ${result.state_id} created!`);
            } else {
